...
import { fetchTokensAndOwners, fetchProjects, scheduleCollectionHolderSnapshot } from '../../../utils/api';
import confetti from 'canvas-confetti';

const NFTSnapshotComponent: React.FC = () => {
  const [collectionId, setCollectionId] = useState<string>('');
  const [collections, setCollections] = useState<Array<{ contractId: number; name: string; }>>([]);
...
.then(data => console.log('Scheduled collection snapshot record response:', data))
.catch((error) => {
  console.error('Error recording snapshot:', error);
      ...
      .then(response => response.json())
      .then(data => console.log('Instant collection snapshot record response:', data))
      .catch((error) => {
        console.error('Error recording snapshot:', error);
      });
    }
      ...
      <Grid numItemsSm={1} numItemsLg={1} className="flex justify-center">
        <Card className="max-w-screen-lg flex flex-col items-center">
          <div className="flex items-center">
            <Metric className="text-center font-pixel ml-16 flex-grow">Collection Holders Snapshot</Metric>
            <img src={hammer} alt="Hammer" className="w-10 h-10 mb-4 ml-4" />
          </div>
          <Divider className='font-bold'>Step 1: Select any ARC-72 Collection</Divider>
          <SearchSelect placeholder="Select Collection" onValueChange={setCollectionId}>
  ...
  {timeOptions.map((option, index) => (
    <SearchSelectItem key={index} value={option.value}>
      {option.label}
          ...
          <Divider className='font-bold mt-10'>{isScheduled ? 'Step 5: Schedule' : 'Step 3: Download'} Holders CSV</Divider>
          <Button 
            size='xl' 
            className='mb-2 w-40' 
            type='button' 
