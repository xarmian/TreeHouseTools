...
for (const chunk of adjustedChunks) {
  ...
  const divisionResult = parsedAmount * multiplicationFactor / ( lpHolders.size );
...
//end metric api stuff
  ...
  return (
    <>
      <Grid numItemsSm={1} numItemsLg={1} className="flex justify-center">
        ...
        <div className="flex items-center">
            <Metric className="mb-4 text-center font-pixel ml-16 flex-grow">Token Holder Airdropper</Metric>
    ...
    <Text className="text-center"><span className="font-bold">{tokenInfo.name} Balance: </span>{formatArc200BalanceAmount(balance, tokenDecimals)}</Text>
           ...
           <Divider className='font-bold'>Step 1: Select an ARC-200 Token</Divider>
                    <SearchSelect placeholder="Select Token" className='' onValueChange={setTokenId}>
                        {lpTokens.map(token => (
    ...
    <Text className="text-center"><span className="font-bold">Token Holders:</span> {isNaN(lpHolders.size) ? 0 : lpHolders.size}
                        </Text>
    <Text className="text-center">
        <span className="font-bold">Amount per Holder:</span> {formatArc200Amount((isNaN(amountPerHolderNFT) ? 0 : amountPerHolderNFT / multiplicationFactor), tokenDecimals)} {tokenInfo.name}
    </Text>
    <Text className="text-center">
...
<Button size='lg' className='mt-6 w-40' onClick={handleSendAlgo} type='button' loading={loading} 
        disabled={loading || (Number(formatArc200Amount((amountPerHolderNFT), tokenDecimals)) == 0) || totalCost === 0 || amountPerHolderNFT === 0 || totalFees === 0 || totalCost / multiplicationFactor >= balance || totalFees >= voiBalance / 1000000}>
  Send it!
</Button>
