...
import { Card, Button, Grid, Text, Metric, Divider, TextInput, DialogPanel, Dialog, ProgressBar, Switch } from '@tremor/react';
import './spinner.css';
...
const AirdropCSVComponent: React.FC = () => {
    ...
    const handleSendAlgo = async () => {
        ...
        try {
            ...
            const amountPerHolder = perHolderAmount ? Amount : Math.floor(Amount / addresses.length);
    ...
    return (
        ...
        <Text>Amount per Address</Text>
                    ...
                    <div className="flex flex-col items-center justify-center space-y-2 md:flex-row md:space-y-0 md:space-x-4 mb-2">
                        ...
                        <Text className="text-center"><span className="font-bold">Address Entries:</span> {isNaN(addresses.length) ? 0 : addresses.length}</Text>
                        <Text className="text-center">
                            <span className="font-bold">Amount per Address:</span> {formatVoiAmount(isNaN(perHolderCost) ? 0 : perHolderCost / 1000000)} VOI
                        </Text>
                        <Text className="text-center">
    <span className="font-bold">Transaction Fees:</span> {formatVoiAmount(isNaN(addresses.length) ? 0 : ( addresses.length ) * 0.001)} VOI
    </Text>
                    </div>
                    <Button size='lg' className='mt-6 w-40' onClick={handleSendAlgo} type='button' loading={loading} disabled={loading || addresses.length === 0 || !amount || totalCost >= (voiBalance - 5000000)}>
                        Send it!
                    </Button>
                        ...
                        <p className='mt-2'><span className='mr-2'>Transaction ID(s):</span>
                            {txIds.map((txId, index) => (
                                <React.Fragment key={txId}>
                                    {index > 0 && ", "}
                                    <Link onClick={() => window.open(`https://voi.observer/explorer/transaction/${txId}`, '_blank')} color="danger">
                                        {txId}
                                    </Link>
                                </React.Fragment>
                            ))}
