...
import { Card, Button, Grid, Text, Metric, Divider, TextInput, DialogPanel, Dialog,  SearchSelect, SearchSelectItem, ProgressBar, Switch } from '@tremor/react';
import './spinner.css';
...
import { fetchTokensAndOwners, fetchProjects, fetchAccountInfo } from '../../../utils/api';
import { formatAddress, formatVoiAmount, truncateTxId } from '../../../utils/formatting';
...
const SendAlgoComponent: React.FC = () => {
  ...
  ? parseInt(amount, 10) * 1000000 * (distributeUnique ? uniqueHoldersCount : uniqueTokensCount) 
  : parseInt(amount, 10) * 1000000;

  const handleSendAlgo = async () => {
    if (!activeAccount || !collectionId || !amount) {
      alert("Please connect your wallet, select a collection, and specify an amount.");
      return;
    }
    ...
    try {
      ...
      const uniqueReceivers = distributeUnique ? [...new Set(receivers)] : receivers;
    
      let txnGroups = [];
      const suggestedParams = await algodClient.getTransactionParams().do();
      let amountsPerReceiver = {}; 
      let totalAirdroppedAmount = 0; 
    
      uniqueReceivers.forEach(receiver => {
        const txnAmount = perHolderAmount
                ? parseInt(amount, 10) * 1000000
                : Math.floor((parseInt(amount, 10) * 1000000) / uniqueReceivers.length);
    
        amountsPerReceiver[receiver] = (amountsPerReceiver[receiver] || 0) + txnAmount;
      ...
      for (let txns of txnGroups) {
        ...
        console.log(`Group sent successfully, first transaction ID: ${sendTxnResponse.txId}`);
      }
...
if (perHolderAmount) {
  ...
  return (
    <>
      <Grid numItemsSm={1} numItemsLg={1} className="flex justify-center">
        ...
        <div className="flex items-center">
            <Metric className="mb-4 text-center font-pixel ml-16 flex-grow">VOI Collection Airdropper</Metric>
    ...
    <Text className="text-center">
        <span className="font-bold">VOI Balance: </span> {formatVoiAmount(isNaN(voiBalance) ? 0 : Math.round(voiBalance / 1000000))}
    </Text>
</div>
      )}
          <Divider className='font-bold'>Step 1: Select a Collection</Divider>
          <SearchSelect placeholder="Select Collection" className='' onValueChange={setCollectionId}>
          ...
          <Divider className='font-bold mt-12'>Step 2: Specify amount to send</Divider>
          <TextInput placeholder="Amount of Voi" onChange={handleInputChange(setAmount)} className='' />
       <div className="flex items-center space-x-3 mt-4">
        ...
        <Divider className='font-bold mt-12'>Step 3: Check details</Divider>
        <div className="flex flex-col items-center justify-center space-y-2 md:flex-row md:space-y-0 md:space-x-4 mb-2">
    <Text className="text-center">
        <span className="font-bold">Total Cost:</span> {formatVoiAmount(isNaN(totalCost) ? 0 : totalCost / 1000000)} VOI
    </Text>
    ...
    <Text className="text-center">
        <span className="font-bold">Amount per {distributeUnique ? "Holder" : "NFT"}:</span> {formatVoiAmount(isNaN(amountPerHolderNFT) ? 0 : amountPerHolderNFT / 1000000)} VOI
    </Text>
    <Text className="text-center">
    <span className="font-bold">Transaction Fees:</span> {formatVoiAmount(isNaN(distributeUnique ? uniqueHoldersCount : uniqueHoldersCount) ? 0 : (distributeUnique ? uniqueHoldersCount : uniqueHoldersCount) * 0.001)} VOI
    </Text>
</div>
          <Button size='lg' className='mt-6 w-40' onClick={handleSendAlgo} type='button' loading={loading}  disabled={loading || collections.length === 0 || totalCost === 0 || totalCost >= ( voiBalance - 5000000 )}>
            Send it!
          </Button>
