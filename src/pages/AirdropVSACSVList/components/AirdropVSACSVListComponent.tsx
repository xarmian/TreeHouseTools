...
const AirdropVSACSVListComponent: React.FC = () => {
  ...
  const handleFileUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      complete: async (results) => {
        ...
        // Check opt-in status for all addresses
        if (tokenInfo.id) {
  ...
  const handleSendVSA = async () => {
    if (
      ...
      alert(
        "Please connect your wallet, select a VSA token, upload a CSV, and specify an amount."
      );
      return;
    }
  ...
  // Check opt-in status when token changes
  useEffect(() => {
    const checkOptInStatus = async () => {
      if (!tokenInfo.id || addresses.length === 0) return;
      ...
      for (const address of addresses) {
        try {
          ...
          console.error(`Error checking opt-in status for ${address}:`, error);
          notOptedIn.push(address);
        }
      }
      ...
      <Grid numItemsSm={1} numItemsLg={1} className="flex justify-center">
        <Card className="flex max-w-screen-lg flex-col items-center">
          ...
          <Text className="mt-2 text-sm text-gray-600">
            Upload a CSV file with one Algorand address per row
          </Text>
          ...
          <div className="mb-2 flex flex-col items-center justify-center space-y-2 md:flex-row md:space-x-4 md:space-y-0">
            ...
            <Text className="text-center">
              <span className="font-bold">Total Addresses:</span>{" "}
              {addresses.length}
            </Text>
            <Text className="text-center">
...
export default AirdropVSACSVListComponent;
