    ...
    const totalCost = perHolderAmount 
        ...
        const handleSendAlgo = async () => {
          if (!activeAccount || !tokenId || !amount || isNaN(parseFloat(amount))) {
              alert("Please connect your wallet, select an LP token, and specify a valid amount.");
              return;
          }
    ...
    return (
        ...
        <Text>Amount per Holder</Text>
                    ...
                    <div className="flex flex-col items-center justify-center space-y-2 md:flex-row md:space-y-0 md:space-x-4 mb-2">
                        ...
                        <Text className="text-center">
        <span className="font-bold">Amount per Holder:</span> {formatVoiAmount(isNaN(perHolderCost) ? 0 : perHolderCost / 1000000)} VOI
    </Text>
    <Text className="text-center">
    <span className="font-bold">Transaction Fees:</span> {formatVoiAmount(isNaN(lpHolders.size) ? 0 : ( lpHolders.size ) * 0.001)} VOI
    </Text>
                    </div>
                    <Button size='lg' className='mt-6 w-40' onClick={handleSendAlgo} type='button' loading={loading} disabled={loading || lpTokens.length === 0 ||  totalCost === 0 || !amount || totalCost >= (voiBalance - 5000000)}>
                        Send it!
                    </Button>
